                          ...
                          <tbody>
                            <?php foreach ($member as $key => $value): ?>
                              <tr>
                                ...
                                <td><a href="<?php echo base_url("admin/manajemen-member/detail/".$value['id_user']); ?>" style="color: #5E5E5E;"><?php echo $value['nama']; ?></a></td>
                                <td><?php echo $value['email']; ?></td>
                                <td><img src="<?php echo base_url("assets/img/member/logo/".$value['logo']); ?>" height="40"></td>
                                <td>
                                  <?php if ($value['dokumen']['status']==0): ?>
                                    <a href="<?php echo base_url("#/$value[id_kategori]"); ?>" class="btn waves-effect m-3 p-5-10" style="color: white; background-color: red;">Belum Valid</a>
                                  <?php elseif ($value['dokumen']['status']==1): ?>
                                    <a class="btn bgm-success waves-effect m-3 p-5-10" style="color: white; background-color: green;">Valid</a>
                                  <?php endif ?>
                                </td>
                                <td>
                                  <?php if ($value['status']==0): ?>
                                    <a class="btn waves-effect m-3 p-5-10" style="color: white; background-color: red;">Non Aktif</a>
                                  <?php elseif ($value['status']==1): ?>
                                    <a class="btn bgm-success waves-effect m-3 p-5-10" style="color: white; background-color: green;">Aktif</a>
                                  <?php endif ?>
                                </td>
                                <!-- <td>
