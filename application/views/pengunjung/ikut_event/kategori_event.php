						...
						<div>
							...
							<select name="id_kategori" class="uk-select" style="border-radius: 5px; border-color: orange;">
								...
								<?php foreach ($kategori as $key => $value): ?>
									<option value="<?php echo $value['id_kategori']; ?>" <?php if($nama_kategori==$value['nama_kategori']){echo "selected";} ?>><?php echo $value['nama_kategori']; ?></option>
								<?php endforeach ?>
							</select>
						</div>
								...
								<?php if (!empty($event1)): ?>
									<?php foreach ($event1 as $key => $value): ?>
										...
										<?php if ($date_now < $date2): ?>
											...
											<div class="uk-width-1-3@m uk-width-1-2@s uk-width-1-1 uk-first-column">
